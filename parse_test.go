...
func TestInvalidFqdn(t *testing.T) {
	...
	assert.Equal(
		t,
		err.Error(),
		"invalid RR: dns: domain must be fully qualified, &dns.MX{Hdr:dns.RR_Header{Name:\"example.com.\", Rrtype:0xf, Class:0x1, Ttl:0xe10, Rdlength:0x0}, Preference:0xa, Mx:\"mail.example.com\"}",
	)
}
